<?php
  ...
  if(!isset($_GET['book']) && isset($_GET['trail'])){
    ...
    $stmt = $con->prepare("SELECT * FROM `paper_trail` WHERE `id` = ? and `book_id` = ? and `requirements` = 0");
    $stmt->bind_param("ii", $_GET['trail'], $_GET['book']);
    $stmt->execute();
    $result = $stmt->get_result();
    //print_r($result);
    if($result->num_rows<1){
      header("Location: " . PROJECT_ROOT . "404.php?msg=The Page you are looking for is not found on this Server.");
    }

    $stmt = $con->prepare("SELECT id FROM `junc_authorbook` WHERE `book_id` = ? and `aut_id` = ?");
    ...
    $result = $stmt->get_result();
    if($result->num_rows<1){
      header("Location: " . PROJECT_ROOT . "404.php?msg=The Page you are lookong for is not found on this Server.");
    }
