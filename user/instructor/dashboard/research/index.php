<?php
  ...
  include PROJECT_ROOT_NOT_LINK . "user/instructor/dashboard/preload.php";
    ...
    $con = $dbconfig ->getCon();
    $stmt = $con->prepare("SELECT book.book_id, book.book_title, book.enabled FROM `junc_authorbook` INNER JOIN book on book.book_id = junc_authorbook.book_id WHERE junc_authorbook.aut_id = ?");
    //print_r($_SESSION);
          ...
          <!-- Toggle Menu  -->
           ...
           <div class="container" style="padding-bottom: 15px; background-color: #e1e1d0; min-height: 40%;">
            ...
            if(count($onGoing)>0){
              ...
              <div class="row title bg-info">
                <div class="col-md-10 col-sm-10 left-holder text-white"  >
                "'. $onGoing[$i]['book_title'] .'"
                </div>
                <div class="col-md-2 col-sm-2 pt-2" style"background-color: red !important" >
                  <a href="paper-status/?book='. $onGoing[$i]['book_id'] .'" class="badge badge-light float-right" style="margin-bottom: 10px;"><i class="fas fa-eye"></i> View</a>
                </div>
              </div>
            </div>';
              }
           ...
           <?php  /*print_r($onGoing); echo $onGoing[0]['book_title'];  */?>
