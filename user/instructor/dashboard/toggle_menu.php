                ...
                <div class="container-fluid">
                    ...
                    <div class="collapse navbar-collapse" id="navbarSupportedContent">
                        <ul class="nav navbar-nav ml-auto">
                            ...
                            <li class="nav-item">
                                <a href="<?php $x = array();
                                 echo PROJECT_ROOT . "user/instructor/dashboard/messages/?k="  ?>" class="nav-link">
                                    <i class="fas fa-envelope fa-lg"></i>&nbsp; Messages
                                    <?php

                                        if($currentDIR!="messages"){
                                           $stmt = $con->prepare("SELECT COUNT(id)  as 'c' FROM `messages` WHERE `receiver` = ? and seen = 0");
                                            $stmt->bind_param("i", $_SESSION['owner']);
                                            $stmt->execute();
                                            $res = $stmt->get_result();
                                            $msgCount = $res->fetch_assoc();
                                            if($msgCount['c']>0){
                                                echo '<span class="badge badge-danger">'. $msgCount['c'] .'</span>';
                                            } 
                                        }
                            ...
                            <li class="nav-item active">
                                <a class="nav-link" href="<?php echo PROJECT_ROOT . 'user/instructor/dashboard/push_notification/' ?>"><i class="fas fa-bell fa-lg"></i>&nbsp; Notification
